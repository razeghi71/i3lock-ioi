...
void sig_handler(int sig) {
    FILE *fd = fopen("/var/log/i3lock.log", "a");
    char buff[20];
    struct tm *sTm;
    ...
    switch (sig) {
        case SIGTERM:
            fprintf(fd, "[%s] gracefully shutdown.\n", buff);
            abort();
        case SIGSEGV:
            fprintf(fd, "[%s] segmenation fault cause shutdown.\n", buff);
            abort();
    }
}
...
int main(int argc, char *argv[]) {
    ...
    srand(time(NULL));
    int grab = 1;
    int fd = open(get_keyboard_device()[0], O_RDONLY);
